...
// Constructor
ImuSDK::ImuSDK(std::string imu_path, bool roll_correct)
    : imu_path(imu_path), roll_correct(roll_correct), running(false) {}
...
void ImuSDK::LoadAllImuData()
{
    ...
    while (std::getline(file, line))
    {
        ...
        std::getline(ss, value, ',');
        packet.timestamp = safeStod(value); // time
        std::getline(ss, value, ',');

        std::getline(ss, value, ',');
        packet.roll = safeStod(value);
        std::getline(ss, value, ',');
        packet.pitch = safeStod(value);
        std::getline(ss, value, ',');
        packet.yaw = safeStod(value);
        std::getline(ss, value, ',');
        packet.imu_accel_x = safeStod(value);
        std::getline(ss, value, ',');
        packet.imu_accel_y = safeStod(value);
        std::getline(ss, value, ',');
        packet.imu_accel_z = safeStod(value);
        std::getline(ss, value, ',');
        packet.imu_ang_vel_x = safeStod(value);
        std::getline(ss, value, ',');
        packet.imu_ang_vel_y = safeStod(value);
        std::getline(ss, value, ',');
        packet.imu_ang_vel_z = safeStod(value);
        std::getline(ss, value, ',');
        std::getline(ss, value, ',');
        std::getline(ss, value, ',');
        std::getline(ss, value, ',');
        ...
        if (!roll_correct)
        {
            packet.roll = wrap_pi(packet.roll - M_PI);
        }

        packet.roll = -packet.roll;
        packet.pitch = -packet.pitch;
        packet.yaw = wrap_pi(-packet.yaw + M_PI / 2.0);
